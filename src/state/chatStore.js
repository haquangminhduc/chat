...
export function createChat(mode = 'standard') { const chat = { id: uid(), title: mode === 'tam_chuyen' ? 'Phòng tám chuyện' : 'Cuộc trò chuyện mới', mode, messages: [], created: Date.now(), pinned: false }; chatStore.set(state => ({ ...state, activeId: chat.id, chats: [chat, ...state.chats] })); return chat; }
