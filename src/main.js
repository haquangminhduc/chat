...
async function send(text, image, clearImage) {
  ...
  chatStore.set(state => ({ ...state, chats: state.chats.map(item => item.id === chat.id ? { ...item, title: hasMessages ? item.title : `${text.slice(0, 42)}${text.length > 42 ? '…' : ''}`, messages: [...item.messages, userMessage, { role: 'assistant', content: '', streaming: true, mood, avatar: '🤔', time: Date.now() }] } : item) }));
  ...
  try {
    ...
    await streamGenerate({ ...settingsStore.get(), system: `${settingsStore.get().system}\n\nDuck Mood hiện tại là ${moodConfig.label}: ${moodConfig.instruction}`, contents, keyManager, signal: controller.signal, onText: textChunk => { streamedText += textChunk; chatArea.updateStreamingText(streamedText); } });
  } catch (error) { if (error.name !== 'AbortError') { const message = error.message || 'Không thể kết nối Duck AI'; streamedText = `Duck AI: ${message}`; finalAvatar = '😵'; toast(message, 'error'); } else { finalAvatar = '😮'; } }
  finally { updateActiveChat(chatItem => ({ ...chatItem, messages: chatItem.messages.map((message, index) => index === chatItem.messages.length - 1 ? { ...message, content: streamedText || 'Duck AI chưa trả về nội dung.', avatar: finalAvatar, streaming: false } : message) })); controller = null; inputBar.setGenerating(false); chatArea.render(); }
}
...
initSidebar({ list: byId('conversationList'), search: byId('searchConversations'), newButton: byId('newChat'), toggleButton: byId('toggleSidebar'), mobileButton: byId('mobileMenu'), clearButton: byId('clearHistory'), backdrop: byId('sidebarBackdrop'), sidebar, onNew: () => { createChat(); closeMobile(); }, onSelect: id => { chatStore.set(state => ({ ...state, activeId: id })); closeMobile(); }, onDelete: chat => { if (!confirm('Xóa cuộc trò chuyện này?')) return; chatStore.set(state => ({ ...state, activeId: state.activeId === chat.id ? (state.chats.find(item => item.id !== chat.id)?.id || null) : state.activeId, chats: state.chats.filter(item => item.id !== chat.id) })); }, onRename: chat => { const title = prompt('Tên cuộc trò chuyện:', chat.title); if (title?.trim()) chatStore.set(state => ({ ...state, chats: state.chats.map(item => item.id === chat.id ? { ...item, title: title.trim() } : item) })); }, onPin: chat => chatStore.set(state => ({ ...state, chats: state.chats.map(item => item.id === chat.id ? { ...item, pinned: !item.pinned } : item) })), onClear: () => { if (!confirm('Xóa toàn bộ lịch sử trò chuyện?')) return; chatStore.set({ activeId: null, chats: [] }); createChat(); toast('Đã xóa toàn bộ lịch sử'); } });
