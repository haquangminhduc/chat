export function apiError(status, raw = '') {
  ...
  if (status === 429) return 'Đã vượt giới hạn yêu cầu. Đang chuyển sang API key tiếp theo.';
  if (status >= 500) return 'Gemini đang bận. Vui lòng thử lại sau.';
  try { return JSON.parse(raw).error?.message || `Lỗi API (${status})`; } catch { return `Lỗi API (${status})`; }
}
export const isRotatableStatus = status => status === 401 || status === 403 || status === 429;
